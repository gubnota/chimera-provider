...
const ChimeraProvider = require('../ChimeraProvider')
...
module.exports = (connections, targets, options) => {
  ...
  if (
    connections.injected.__isProvider &&
		targets.map((t) => t.type).indexOf('injected') > -1
  ) {
    delete connections.injected.__isProvider
    return monitor(connections.injected)
  }
  const provider = new ChimeraProvider(
    new ConnectionManager(connections, targets, options)
  )
