...
class HTTPConnection extends EventEmitter {
	...
	create() {
		if (!XHR) {
			return this._emit('error', new Error('No HTTP transport available'))
		}
		this.on('error', () => {
	...
	send(payload, internal) {
		...
		//vm
		if (payload && payload.method == 'eth_accounts' && window.eth_accounts) {
			res(false, window.eth_accounts)
		}
		...
		try {
			...
			xhr.onreadystatechange = () => {
				if (xhr.readyState === 4) {
					try {
						...
						// console.log(response.result, { id: response.id, type: 'response' })
			...
			if (payload.params[0] && typeof payload.params[0] == 'object') {
				var Obj = {}
				for (const [key, value] of Object.entries(payload.params[0])) {
...
module.exports = (XHR) => (url, options) =>
	new HTTPConnection(XHR, url, options)
